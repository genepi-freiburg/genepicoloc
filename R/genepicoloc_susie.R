...
create_LD_matrix <- function(sumstats, bfile, fixref=F, keep=NULL,
  ...
  if (fixref) {
    ...
    system(paste0(plink2_cmd, " --extract ", tmp, " --ref-allele ", tmp,
  ...
  if (sum(to_remove_row) > nrow(r_mat)/5) {
    warning("Removed more than 20% of variants, input genotypes might have high missingness rate. Please check the input and filter with --geno and --mind, if necessary.")
  }
  if (sum(to_remove_row) > 4*nrow(r_mat)/5) {
    stop("Removed more than 80% of variants, input genotypes have very high missingness rate. Please check the input and filter with --geno and --mind.")
  }
...
genepicoloc_susie <- function(raw_gt, pheno, pheno_name,
  ...
  # run susie()
  susie_out <- susie(raw_gt_mat, pheno_mat,
                     coverage=coverage,
                     verbose = TRUE)
  return(susie_out)
}
...
genepicoloc_susie_rss <- function(sumstats, LD_matrix,
                                  ...
                                  N, coverage=0.95, ...) {
  # checks
  stopifnot(all(sumstats[[Name_name]] == colnames(LD_matrix)))
  stopifnot(all(sumstats[[Name_name]] == rownames(LD_matrix)))
  if (missing(N)) N <- median(sumstats[[N_name]], na.rm = T)
  if (is.na(N) | is.null(N)) stop ("Sample size is missing")
  # run susie_rss
  fitted_rss <- susie_rss(bhat = sumstats[[BETA_name]],
...
susie_rss_wrapper <- function(sumstats, bfile, N=NULL, coverage=0.95,
                              rm_indels = F) {
  if (rm_indels) {
    idx_indels <- !(sumstats$A1 %in% c("A", "T", "G", "C") & 
                      sumstats$A2 %in% c("A", "T", "G", "C"))
    sumstats <- sumstats[!idx_indels,]
  }
  if (is.null(N)) N <- median(sumstats$N, na.rm = T)
  LD_matrix <- create_LD_matrix(sumstats, bfile=bfile, fixref=T)
  sumstats <- subset(sumstats, Name %in% colnames(LD_matrix))
  susie_obj <- genepicoloc_susie_rss(sumstats=sumstats,
                                     LD_matrix=LD_matrix,
                                     N = N,
                                     coverage=coverage)
  return(susie_obj)
